...
from ats_clients import fetch_greenhouse, fetch_lever, fetch_workday, fetch_ashby
...
def fetch_company_jobs(entry):
    ...
    try:
        if ats == "greenhouse":
            ...
            return fetch_workday(name, entry["tenant"], entry["site"], wd_num=entry.get("wd_num"))
        elif ats == "ashby":
            return fetch_ashby(name, entry["job_board_name"])
        elif ats == "custom":
