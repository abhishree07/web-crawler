...
### Dry run mode (recommended for your first test)

Click "Run workflow" and check the **"Dry run"** checkbox before starting
it. This fetches and filters jobs exactly like a real run, but only prints
the matches to the workflow log instead of sending Telegram messages, and
doesn't mark anything as "seen" — so it's safe to run repeatedly while
you're checking that everything works.

To see the output: after the run finishes, click into it, click the
"Run job scraper (dry run)" step, and expand the log.

Once you're happy with what it finds, run the workflow again with "Dry
run" unchecked — that's a real run that sends Telegram messages.
